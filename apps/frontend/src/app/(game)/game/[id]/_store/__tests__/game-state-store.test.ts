...
const GAME_MOCK = generateGame();
const MASTER_PARTICIPANT_MOCK = generateParticipant({
...
describe("Game State Store", () => {
	test("has a correct initial value", async () => {
		const { result } = renderHook(() =>
			createGameStateStore({
				game: GAME_MOCK,
				currentParticipant: MASTER_PARTICIPANT_MOCK,
			}),
		);
	...
	describe("Game Async State Slice", () => {
		...
		test("updateCurrentParticipant - updates currentParticipant", async () => {
			const currentParticipant = MASTER_PARTICIPANT_MOCK;
			const { result } = renderHook(() =>
				createGameStateStore({
					game: {
						...GAME_MOCK,
